...
export default function SoftwarePage() {
  ...
  const loadSoftwareInventory = async () => {
    ...
    try {
      ...
      // 1. Look for SBOM data (most comprehensive)
      const sbomData = agentData.find((data: any) => 
        data.message_type === 'sbom' && 
        data.data && 
        data.data.software_sources
      )
      
      if (sbomData) {
        console.log('🎯 Found SBOM data:', sbomData)
        const sbomSoftware = sbomData.data
        
        // Extract software from all SBOM sources
        if (sbomSoftware.software_sources && Array.isArray(sbomSoftware.software_sources)) {
          for (const source of sbomSoftware.software_sources) {
            if (source.packages && Array.isArray(source.packages)) {
              console.log(`📦 Processing SBOM source: ${source.source_type} with ${source.packages.length} packages`)
              
              const sourceItems = source.packages.map((item: any) => ({
                name: item.DisplayName || item.Name || item.FeatureName || item.PackageName || 'Unknown',
                version: item.DisplayVersion || item.Version || 'Unknown',
                publisher: item.Publisher || item.Vendor || 'Unknown'
              }))
              
              softwareData = softwareData.concat(sourceItems)
            }
          }
          inventoryTimestamp = sbomData.timestamp || new Date().toISOString()
          console.log(`✅ Extracted ${softwareData.length} software items from SBOM data`)
        }
      }
      
      // 2. Look for standard inventory data (if no SBOM data found)
