      ...
      try {
        ...
        if (hardeningResults.length > 0) {
          ...
          if (hardeningData) {
            ...
            console.log('📝 No hardening_data but found HardeningKitty output, parsing...')
                      ...
                      .map(([id, agent]: [string, any]) => (
                        <SelectItem key={id} value={id}>
                          <div className="flex items-center gap-2">
                            ...
                            {agent.hostname || id.substring(0, 12)} ({agent.os ? `${agent.os}${agent.os_version ? ` ${agent.os_version}` : ''}` : 'Unknown'})
                          </div>
                        </SelectItem>
                      ))
                  )}
